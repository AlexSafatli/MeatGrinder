...
class NoteInserter:
    def __init__(self, locator):
        ...
        for i in notes[locator.location[-1]]:
            ...
            if hasattr(self, 'note_%d' % i):
                getattr(self, 'note_%d' % i)()
    ...
    def note_1(self):
        if 'Abdomen' not in self.loc.location[0]:
            self.notes.append(
                "Attacks that miss by 1 hit the <strong>Torso</strong> instead."
            )

    def note_2(self):
        self.notes.append(
            '<strong>Eyes</strong> are treated as <strong>Skull</strong> but '
            'without DR 2.')
        self.notes.append(
            'Any armor covering the face will stop an <strong>Eye</strong> '
            'hit unless the blow specifically targets this sub-location.')
        self.note_3()
    ...
    def note_4(self):
        ...
        if loc.atktype == 'cor':
            loc.multiplier = 1.5
            self.notes.append(
                'Major wound blinds one <strong>Eye</strong>. Both for >HP.')
        self.notes.append(
            'Random attack from behind hit the <strong>Skull</strong>.')
    ...
    def note_6(self):
        self.notes.append(
            'If holding a shield, -4 to hit for <strong>Arm</strong>, -8 to '
            'hit for <strong>Hand</strong>.')
    ...
    def note_15(self):
        ...
        if loc.atktype == 'cut':
            self.notes.append(
                'HP/2 severs the <strong>Nose</strong>. No knockdown penalty. '
                'Appearance -2.')
        self.notes.append(
            'HP/4 cause <em>No Sense of Taste or Smell</em> until healed.')
